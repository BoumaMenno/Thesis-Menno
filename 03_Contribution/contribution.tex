...
\begin{myass}[Existence of a guard function, perturbation-independent guard-event pairs]\label{ass:existence}
...
\end{array}
\end{equation}
where $B_{\varepsilon_{\gamma}}(\alphab(\tau,j),\tau)$ is a ball with radius $\varepsilon_{\gamma}$ around $\alphab(\tau,j)$ with $\tau = \tau^{j+1}$. $\mathcal{C}^j$ is the flow set of a trajectory after event $j$ and $\mathcal{D}^{j+1}$ is the event set which triggers event $i+1$, which is defined as $\partial \mathcal{C}^j$, where $\partial \mathcal{C}^j$ represents the boundary of flow set $\mathcal{C}^j$ (existence of a guard function).
\end{myass}

\begin{myass}[Perturbation-independent guard-event pairs]
We assume that there exist constants $\varepsilon_\gamma$, and real valued guard function $\gamma(\xb,\ub,t,j)$ which is continuously differentiable with respect to $\xb$, $\ub$, and $t$, for each $j\in \{1,2,...,N\}$, such that We now define the set of open guard functions $\gamma_{\text{open}}$.
\begin{align}
\gamma_{\text{open}}\setminus\gamma(\xb,\ub,t,j)
\end{align}
\end{myass}
