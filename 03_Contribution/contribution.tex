...
\begin{myass}[Guard-transition pairing]\label{ass:guardtrans}
...
\end{subfigure}
\caption{The state evolution of the nominal and state trajectory. Note that the state trajectory first triggers $\gamma^{\textrm{cl}\rightarrow\textrm{op}}$, but then enters a new flow set where $\gamma^{\textrm{cl}\rightarrow\textrm{op}}$ is still participating. This is possible because $\gamma^{\textrm{cl}\rightarrow\textrm{op}}$ is defined on acceleration level.}
\label{fig:4example}
\end{figure}
...
\begin{myass}[Unidirectional guard activation]\label{ass:unidirectional}
