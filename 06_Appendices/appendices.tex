...
\end{equation}
The flow dynamics can then be described as
\begin{align}
...
&\zetab_{t,i} = \kappa_i\epsilon(\lambdab_{t,i}),\label{eq:ncpcontact4}
\end{align}
with 
\begin{align}
...
\end{align}
Here \eqref{eq:ncpcontact2} is called the contact law and \eqref{eq:ncpcontact3} and \eqref{eq:ncpcontact4} the friction law, which are the complementarity conditions that describe the unilateral constraints and spatial friction. When the system activates a unilateral constraint, a jump in the state can take place. This jump in the state is described by the impact dynamics
\begin{align}
...
\end{align}
These dynamics are impulsive, and happen at one instance in time. The $^-$ superscript indicates the ante-event state and the $^+$ superscript indicates the post-event state.
...
\end{align}
and the mode transition from open to stick happens when a guard is triggered at velocity level
\begin{align}
