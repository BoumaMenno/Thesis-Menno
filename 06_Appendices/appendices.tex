...
When a contact point is in closed-contact slip, it can go to closed-contact stick and it can go to open-contact. A slipping contact transitions to sticking when the tangential velocity of the contact point is zero, i.e.,
...
we define
\begin{align}
&\ddot{\qb} = \Mb^{-1}\left[\Sb\ub - \Hb + \Wb_{n}\lambdab_{n} + \Wb_{t}\lambdab_{t}\right],\label{eq:appqddot}\\
&\Wb^T_{n}\ddot{\qb} + \dot{\Wb}^T_{n}\dot{\qb} = 0,\\
&\lambdab_{t} = -\mu\lambdab_{n}\odot\langle\zetab_{t}\rangle,
\end{align}
where $\odot$ indicates the element wise multiplication of two vectors and $\langle\zetab_{t}\rangle=\begin{bmatrix}\langle\zetab_{t,i_1}\rangle,\langle\zetab_{t,i_2}\rangle,...,\langle\zetab_{t,i_C}\rangle \end{bmatrix}^T$, with $\langle\zetab_{t,i_c}\rangle$ the unit vector $\zetab_{t,i_c}$.
