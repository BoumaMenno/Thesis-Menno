...
\begin{multline}
\ls^{s^{k+1}}\zb(\tau) = D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb\Delta^{k+1} - D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k+1}}\fb\Delta^{k} + D_1\ls^{s^{k+1}}\gb\cdot\left(D_1\ls^{s^{k}}\gb\cdot\left(\ls^{s^{k-1}}\zb+\ls^{s^{k-1}}\fb\Delta^{k}\right)\right.\\
...
&D_1\ls^{s^{k+1}}\gb\cdot\left(-\ls^{s^{k}}\fb\Delta^{k}\right) = \frac{D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb}{\dot{\gamma}^{k}}\left(D_1\gamma^{k}\cdot\ls^{s^{k-1}}\zb + D_2\gamma^{k}\cdot\ls^{s^{k-1}}\vb\right),
\end{align}
\begin{multline}
D_1\ls^{s^{k+1}}\gb\cdot\left(D_1\ls^{s^{k}}\gb\cdot\left(\ls^{s^{k-1}}\zb + \fb^{k-1}\Delta^{k}\right) + D_2\ls^{s^{k}}\gb\cdot\left(\ls^{s^{k-1}}\vb+\dot{\mub}^{k-1}\Delta^{k}\right) + D_3\ls^{s^{k}}\gb\cdot\Delta^{k}\right) = \\
D_1\ls^{s^{k+1}}\gb\cdot\left(\frac{-D_1\ls^{s^{k}}\gb\cdot\fb^{k-1}}{\dot{\gamma}^{k}}D_1\gamma^{k} + \frac{-D_2\ls^{s^{k}}\gb\cdot\dot{\mub}^{k-1}}{\dot{\gamma}^{k}}D_1\gamma^{k} + \frac{-D_3\ls^{s^{k}}\gb\cdot 1}{\dot{\gamma}^{k}}D_1\gamma^{k} + D_1\ls^{s^{k}}\gb\right)\cdot\ls^{s^{k-1}}\zb \\
+ D_1\ls^{s^{k+1}}\gb\cdot\left(\frac{-D_1\ls^{s^{k}}\gb\cdot\fb^{k-1}}{\dot{\gamma}^{k}}D_2\gamma^{k} + \frac{-D_2\ls^{s^{k}}\gb\cdot\dot{\mub}^{k-1}}{\dot{\gamma}^{k}}D_2\gamma^{k} + \frac{-D_3\ls^{s^{k}}\gb\cdot 1}{\dot{\gamma}^{k}}D_2\gamma^{k} + D_2\ls^{s^{k}}\gb \right)\cdot \ls^{s^{k-1}}\vb,
\end{multline}
\begin{align}
&D_2 \ls^{s^{k+1}}\gb\cdot\left(\ls^{s^{k}}\vb+\ls^{s^{k}}\dot{\mub}\Delta^{k+1} \right) = D_2\ls^{s^{k+1}}\gb\left(-\frac{\ls^{s^k}\dot{\mub}}{\dot{\gamma}^{k+1}}D_1\gamma^{k+1}\cdot\ls^{s^k}\zb + \left(1 - \frac{\ls^{s^k}\dot{\mub}}{\dot{\gamma}^{k+1}}D_2\gamma^{k+1}\right)\cdot\ls^{s^k}\vb \right),\\
...
\end{align}
\textbf{REFORM $D_2 \ls^{s^{k+1}}\gb$ EQUATION, THEN CONTINUE}
When we substitute the equations above into \eqref{eq:z2}, after reordering the expression we get

\begin{multline}
\zb^{k+1}(\tau) = \left(\frac{\ls^{s^{k+1}}\fb - D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb - D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_1\gamma^{k+1}\cdot\Gb^{k}\right.\\
\left. + D_1\ls^{s^{k+1}}\gb\cdot\frac{\ls^{s^{k}}\fb - D_1\ls^{s^{k}}\gb\cdot\fb^{k-1} - D_2\ls^{s^{k}}\gb\cdot\dot{\mub}^{k-1} - D_3\ls^{s^{k}}\gb\cdot 1}{\dot{\gamma}^{k}}D_1\gamma^{k+1} + D_1\ls^{s^{k+1}}\gb D_1\ls^{s^{k}}\gb\cdot 1\right)\ls^{s^{k-1}}\zb\\
+ \left(\frac{\ls^{s^{k+1}}\fb-D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb -D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_1\gamma^{k+1}\cdot\Jb^{k}\right.\\
\left. + D_1\ls^{s^{k+1}}\gb\cdot\frac{\ls^{s^{k}}\fb-D_1\ls^{s^{k}}\gb\cdot\fb^{k-1}-D_2\ls^{s^{k}}\gb\cdot\dot{\mub}^{k-1}-D_3\ls^{s^{k}}\gb\cdot 1}{\dot{\gamma}^{k}}D_2\gamma^{k+1} + D_1\ls^{s^{k+1}}\gb D_2\ls^{s^{k}}\gb\cdot 1 \right)\ls^{s^{k-1}}\vb\\
+ \left(\frac{\ls^{s^{k+1}}\fb-D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb - D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_2\gamma^{k+1}\right)\ls^{s^{k}}\vb,
\end{multline}
from which we can isolate $\Gb^{k}$, and $\Jb^{k}$ resulting in
\begin{multline}
\zb^{k+1}(\tau) = \left(\frac{\ls^{s^{k+1}}\fb - D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb  - D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_1\gamma^{k+1} + D_1\ls^{s^{k+1}}\gb\cdot 1\right)\Gb^{k}\ls^{s^{k-1}}\zb\\
+ \left(\frac{\ls^{s^{k+1}}\fb - D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb - D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_1\gamma^{k+1} + D_1\ls^{s^{k+1}}\gb\cdot 1\right)\Jb^{k}\ls^{s^{k-1}}\vb \\
+ \left(\frac{\ls^{s^{k+1}}\fb-D_1\ls^{s^{k+1}}\gb\cdot\ls^{s^{k}}\fb - D_2\ls^{s^{k+1}}\gb\cdot\dot{\mub}^{k} - D_3\ls^{s^{k+1}}\gb\cdot 1}{\dot{\gamma}^{k+1}}D_2\gamma^{k+1}\right)\ls^{s^{k}}\vb,
\end{multline}
which is equal to
\begin{align}
&\zb^{k+1}(\tau) = \Gb^{k+1}\Gb^k\ls^{s^{k-1}}\zb + \Gb^{k+1}\Jb^k\ls^{s^{k-1}}\vb + \Jb^{k+1}\ls^{s^{k}}\vb.\label{eq:zkplusfinal}
\end{align}
...
\ ^{^{(12)} S^{2\leftarrow 0}}\Gb\ ^{s^0}\zb + \ ^{^{(12)} S^{2\leftarrow 0}}\Jb\ ^{s^0}\vb, & \text{if }(\text{I}),\\
\ ^{^{12} S^{2\leftarrow 1}}\Gb\ ^{^{1} S^{1\leftarrow 0}}\Gb\ ^{s^0}\zb + \ ^{^{12} S^{2\leftarrow 1}}\Gb\ ^{^{1} S^{1\leftarrow 0}}\Jb\ ^{s^0}\vb + \ ^{^{12} S^{2\leftarrow 1}}\Jb\ ^{^1 S^1}\vb, & \text{if }(\text{II}),\\
\ ^{^{21} S^{2\leftarrow 1}}\Gb\ ^{^{2} S^{1\leftarrow 0}}\Gb\ ^{s^0}\zb + \ ^{^{21} S^{2\leftarrow 1}}\Gb\ ^{^{2} S^{1\leftarrow 0}}\Jb\ ^{s^0}\vb + \ ^{^{21} S^{2\leftarrow 1}}\Jb\ ^{^{2} S^1}\vb, & \text{if }(\text{III}),
