...
\chapter{Friction in Mechanical Systems with Unilateral Constraints}
\section{Frictional impacts}
\begin{itemize}
	\item How does transversality wrt $\Gamma$ translate to the behaviour in flow?
    \item Are the contacts non-superfluous?
	\item Are the reset maps associative?
\end{itemize}

\section{Reference trajectories with impact away from slip-stick border}
...
The mode transition from stick to slip happens when a guard is triggered at acceleration level, i.e., $^{\text{slip}\leftarrow\text{stick}}\gamma = \mu^2\lambda_{n,i}^2 - \lambdab_{t,i}\lambdab_{t,i}^T$. The jump from open to stick is described on velocity level by the jump map
...
This implies that it is possible for the post-impact state of an open-to-stick transition to be in the stick-to-slip jump set, causing an immediate transition to slip. The flow dynamics of the stick mode are described as

\begin{align}
&\Mb(\qb)\ddot{\qb} + \Hb(\qb,\dot{\qb}) = \Sb(\qb)\ub + \sum_{i\in\Ic_c}\left(\wb_{n,i}(\qb)\lambda_{n,i} + \Wb_{t,i}(\qb)\lambdab_{t,i} \right), \label{eq:appstickdyn1}\\
...
\end{align}
 \textbf{Show using dynamics above that the instant guard trigger is possible. Then create image of the state + input space with stick to slip guard and illustrate a post open-to-stick state. Say that both areas are viable options. Then make similar proof of how we can stay away from that border. Explain that we have to stay away from border because we do not know what the dynamics are at that point (so different to simultaneous event, where we do know the dynamics!!).}
