...
When a contact point is in closed-contact stick, it can transition to closed-contact slip and it can transition to open-contact. A slipping contact transitions to sticking when the tangential reaction force of the contact point is zero, i.e.,
...
\begin{align}
\gamma_{\text{st}\rightarrow\text{sl}} = \lambdab_{t,i_c}\lambdab_{t,i_c}^T,
\end{align}

which is equal to zero when $||\lambdab_{t,i_c}|| = 0$, greater than zero when $||\lambdab_{t,i_c}|| > 0$, smaller than zero when $||\lambdab_{t,i_c}|| < 0$, and it is globally differentiable. The dynamics and constraints of a system with all contact points in stick are defined as
...
Using \eqref{eq:applambdastick} the guard function $\gamma_{\text{st}\rightarrow\text{sl}} = \lambdab_{t,i_c}\lambdab_{t,i_c}^T$ is defined.
